...
(defn rev-add-seq [n]
  (let [sum (+ n (reverse-digits n))]
    (lazy-seq 
      (cons sum (rev-add-seq sum)))))

(defn is-lychrel-number? [n]
  (not-any? is-palindrome? (take 50 (rev-add-seq (bigint n)))))

(def lychrel-seq 
  (filter is-lychrel-number? (iterate inc 1)))

(defn solve [lim]
  (count (take-while #(< % lim) lychrel-seq)))
